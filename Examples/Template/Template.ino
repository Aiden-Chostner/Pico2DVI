  ...
  for (int y = 0; y < display.height(); y++) {
    for (int x = 0; x < display.width(); x++) {
      if (!(x < 170 && y < 66)) { //draw every pixel except on overlay to prevent flickering
        ...
        delay(2);
        SPIM(MCS0, 0, 0x06, 0, 0);
        SPIM(MCS0, 2, 0x02, addressRandom(index+1), (uint8_t)get_rand_32());
        delay(2);
        display.drawPixel(x, y, SPIM(MCS0, 1, 0x03, addressRandom(index), 0)*256+SPIM(MCS0, 1, 0x03, addressRandom(index+1), 0));
      }
    }
  }
